...
SNAPSHOT_MODULES = [
    ...
    SNAPSHOT_PANEL.parent / "SnapshotPanel" / "snapshotViews.ts",
    SNAPSHOT_PANEL.parent / "SnapshotPanel" / "SnapshotPanel.tsx",
    ...
    SNAPSHOT_PANEL.parent / "SnapshotPanel" / "SnapshotDesktopTable.tsx",
    SNAPSHOT_PANEL.parent / "SnapshotPanel" / "SnapshotMobileCard.tsx",
]


def test_snapshot_panel_declares_all_data_views_and_endpoints():
    source = "\n".join(resolve_react_source(path) for path in SNAPSHOT_MODULES)
    ...
    assert 'aria-label={`${title}表格`}' in source
    assert 'aria-label={`${title}移动列表`}' in source
    ...
    assert "label: '缓存总数'," in source
    assert "value: text(summary.total ?? summary.count ?? summary.total_count ?? '-')," in source
