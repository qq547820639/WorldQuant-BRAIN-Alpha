...
CANDIDATE_TABLE_MODULES = [
    ...
    REACT_SRC / "components" / "CandidateTableToolbarFilterToolbar.tsx",
    REACT_SRC / "components" / "CandidateTableToolbarProductionControls.tsx",
    REACT_SRC / "components" / "CandidateTableToolbarQualitySummaryBar.tsx",
    REACT_SRC / "components" / "CandidateTableToolbarTitleStats.tsx",
    REACT_SRC / "components" / "CandidateTableSubComponents.tsx",
    REACT_SRC / "components" / "CandidateTableSubComponents",
    REACT_SRC / "components" / "CandidateTableUtils.ts",
    REACT_SRC / "components" / "CandidateTableUtils",
    REACT_SRC / "components" / "CandidateTableDesktop.tsx",
    REACT_SRC / "components" / "CandidateTableMobile.tsx",
    REACT_SRC / "components" / "CandidateTableLoading.tsx",
    REACT_SRC / "components" / "CandidateTableSuccessBanner.tsx",
    ...
    REACT_SRC / "hooks" / "useCandidateTableState.ts",
    REACT_SRC / "hooks" / "useCandidateTableData.ts",
    REACT_SRC / "hooks" / "useCandidateTableSse.ts",
    REACT_SRC / "hooks" / "useCandidateGeneration.ts",
    REACT_SRC / "hooks" / "useCandidateSimulation.ts",
    REACT_SRC / "hooks" / "useCandidateOptimization.ts",
    REACT_SRC / "hooks" / "useCandidateCheck.ts",
    REACT_SRC / "hooks" / "useSseManager.ts",
    REACT_SRC / "hooks" / "useGlobalData.ts",
]


def _source() -> str:
    return "\n".join(resolve_react_source(path) for path in CANDIDATE_TABLE_MODULES)


def _app_source() -> str:
    return "\n".join([
        resolve_react_source(APP),
...
def test_candidate_table_fetches_all_candidates_and_uses_local_pagination_window():
    ...
    assert "candidatesApi.call('/api/candidates')" in source
    assert "CANDIDATE_FETCH_LIMIT" not in source
    assert "?limit=1000" not in source
    assert "export const PAGE_SIZE = 20;" in source
...
def test_candidate_table_bounds_target_pool_size_and_sanitizes_filter_input():
    ...
    assert "setFilterInput(sanitizeTextInput(value, MAX_FILTER_LENGTH));" in source
...
def test_candidate_pool_updates_refresh_phase_state_in_app_shell():
    ...
    assert "void candidatesApi.call('/api/candidates');" in app
    assert "globalData.refreshAll();" in app
    assert "void phaseApiCall('/api/phase_state');" in app
    assert "onCandidatePoolUpdated: appState.handleCandidatePoolUpdated" in app
...
def test_candidate_table_sort_headers_expose_column_sort_state():
    ...
    assert "{active ? (sortAsc ?" in source
...
def test_candidate_table_tolerates_sparse_lifecycle_rows_and_uses_all_candidate_ids():
    ...
    assert "function candidateIds(" in source
    assert "Pick<Candidate, 'alpha_id' | 'official_alpha_id' | 'simulation_id'>" in source
    ...
    assert "const normalized = candidateText(" in source
    assert "candidate.lifecycle_status" in source
...
def test_candidate_table_exposes_alpha_quality_diagnostics_and_output_config():
    ...
    assert "function summarizeCandidateQuality(" in source
    assert "candidates: Candidate[]" in source
    assert "retained: number" in source
    assert "targetPoolSize: number" in source
    ...
    assert "candidateManagementDisplayCandidates(" in source
    assert "retainedPoolCandidates" in source
    assert "serverWorkflowPlan" in source
    assert "rawQueueCandidates" in source
...
def test_candidate_table_exposes_queue_view_filters_for_inline_parity():
    ...
    assert "if (viewMode === 'backtest_rework')" in source
    assert "return status === 'backtest_rework' || status === 'failed_backtest' || status === 'rejected';" in source
    ...
    assert "if (viewMode === 'submittable')" in source
...
def test_candidate_table_loads_fresh_check_results_for_submittable_queue():
    ...
    assert "result?.is_stale !== true" in source
    assert "result?.submittable ?? result?.passed ?? candidate.quality_diagnosis?.submission_ready" in source
...
def test_candidate_table_auto_pool_simulates_top_three_main_pool_ids():
    ...
    assert "serverMainPoolCandidates" in source
    assert "rankPoolCandidates(serverMainPoolCandidates)" in source
    assert "const hasExplicitOverride = Boolean(candidateOverride && candidateOverride.length);" in source
    assert "workflowCandidatesForQueue(" in source
    assert "serverWorkflowPlan?.validator?.next_candidate_ids" in source
...
def test_candidate_table_only_runs_batch_check_after_successful_simulation():
    ...
    assert "pipeline.autoPipelineStageRef.current === 'await_quality_check'" in source
    assert "simulationSucceeded" in source
...
def test_candidate_table_refills_after_quality_check_deficit():
    ...
    assert "candidatePoolSnapshot(" in source
    assert "nextRows" in source
    assert "nextMainPool" in source
    assert "nextWorkflowPlan" in source
    assert "const producerDeficit = Number(workflowPlan?.producer?.deficit);" in source
    assert "const shouldContinueMaintenance =" in source
    assert "pipeline.autoPipelineStageRef.current === 'await_quality_check'" in source
    ...
    assert "shouldContinueMaintenance" in source
    assert "loaded?.snapshot.deficit" in source
    assert "loaded.snapshot.deficit > 0" in source
    assert "void generateCandidates(loaded.snapshot)" in source
    assert "主池仍缺 ${loaded.snapshot.deficit} 个候选，继续自动补位。" in source
    assert "resetAutoPipelineStageIfCurrent('await_quality_check');" in source
    assert "onGenerateCandidates={() => {" in source
    assert "void actions.generateCandidates();" in source
    assert "onRetryTask: () => {" in source
...
def test_candidate_table_optimizes_rework_before_refill_without_submit_or_credentials():
    ...
    assert "autoOptimizationCycles < maxAutoOptimizationCycles" in source
    assert "void startOptimization(loaded.snapshot, reworkCandidates);" in source
    assert "先优化 ${Math.min(reworkCandidates.length, autoSimulationBatchSize)} 个需优化候选" in source
    ...
    assert "const startBatchCheck = useCallback(" in source
    assert "async (candidateOverride?: Candidate[])" in source
    assert "const candidatesForCheck =" in source
    assert "candidateOverride && candidateOverride.length" in source
    assert "void startBatchCheck(cForCheck);" in source
