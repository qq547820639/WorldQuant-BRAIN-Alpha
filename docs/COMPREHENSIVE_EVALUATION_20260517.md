...
| **M-06** | 官方 API 完整分页需停滞观测 | `official.py:174-192, 212-230, 252-269` | 上游异常时可能重复写缓存或长期运行 | ① 保留云端 Alpha 完整分页；② 记录上一页 ID 哈希检测重复；③ 显示停滞/取消原因 | 分页循环通过重复页、空页/短页、显式取消和真实 API 错误保护，不用固定截断 |
