...
class ScoreHistoryDB:
    ...
    def convergence_stats(self, *, limit: int = DEFAULT_HISTORY_LIMIT) -> dict[str, Any]:
        ...
        scores = [r.get("total_score", 0) for r in records if "total_score" in r]  # B-06: defensive .get()
        ...
        return {
            ...
            "pass_rate": round(
                # B-06: defensive .get()
                sum(1 for r in records if r.get("passed_gate")) / len(records),
                3,
            ),
            "api_zero_deviation_rate": round(
                # B-06: defensive .get()
                sum(1 for r in records if r.get("api_deviation") == 0.0) / len(records),
                3,
            ),
        }
