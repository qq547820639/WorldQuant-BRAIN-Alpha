...
_FUSION_MAX_EXPR_LENGTH = 512
_FUSION_MAX_PAREN_DEPTH = 12


def _validate_fusion_expr(expr: str, label: str = "fusion") -> str | None:
    """Validate a fusion-generated expression for BRAIN platform limits.

    Returns the expression if valid, or None if it exceeds platform limits.
...
def orthogonal_blend(alpha1_expr: str, alpha2_expr: str) -> str:
    ...
    return _validate_fusion_expr(expr, "orthogonal_blend")
...
def residual_alpha(base_expr: str, signal_expr: str) -> str:
    ...
    return _validate_fusion_expr(expr, "residual_alpha")
...
def composite_fusion(ex1: str, ex2: str, mode: str = "orthogonal") -> str:
    ...
    if mode == "orthogonal":
        ...
        expr = f"rank({ex1}) * rank({ex2})"
        return _validate_fusion_expr(expr, "composite")
    else:
...
def composite_ensemble(expressions: list[str], mode: str = "average") -> str:
    ...
    if mode == "average":
        ...
        expr = f"({terms}) / {len(expressions)}"
        return _validate_fusion_expr(expr, "ensemble_average")
    elif mode == "rank_average":
        terms = " + ".join(f"rank({e})" for e in expressions)
        expr = f"({terms}) / {len(expressions)}"
        return _validate_fusion_expr(expr, "ensemble_rank_average")
    elif mode == "max":
        ...
        for e in expressions[2:]:
            result = f"min(({result}), ({e}))"  # B-04: BRAIN vector max/min
        return _validate_fusion_expr(result, "ensemble_min")
    else:
