"""Recording of Alpha official backtest features and AB comparison.

Consolidated from the original ``experience`` subpackage. Holds
``record_alpha_result`` (writes to alpha_features.jsonl), the private
``_record_ab_comparison`` (writes to ab_tests.jsonl), and
``get_mutation_effectiveness`` (summarizes AB improvement by mutation type).
