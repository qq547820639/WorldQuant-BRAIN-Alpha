"""Bootstrap and trend-analysis mixin for ``ConvergenceTracker``.

Extracted from the original ``convergence.py`` monolith. The
``_bootstrap_ci`` (BCa bootstrap confidence interval) and ``_spearman_trend``
(Spearman rank correlation trend detection) methods live here. They are
mixed into ``ConvergenceTracker`` in ``_tracker`` to keep the main class
file under the per-submodule line budget while preserving the public
class API.
"""

from __future__ import annotations

from brain_alpha_ops.research.convergence._stats import _bca_alpha, _inv_norm_cdf
from brain_alpha_ops.research.convergence._types import CycleRecord
