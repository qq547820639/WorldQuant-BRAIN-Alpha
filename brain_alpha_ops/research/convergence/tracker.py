...
Consolidated from the original ``convergence.py`` monolith. The
``_bootstrap_ci`` and ``_spearman_trend`` statistical helpers are mixed
in via ``_BootstrapMixin`` (see ``bootstrap``) to keep this file focused
on the public ``ConvergenceTracker`` class while preserving the public API.
"""
