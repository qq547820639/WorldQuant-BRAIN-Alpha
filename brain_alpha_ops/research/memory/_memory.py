...
Extracted from the original ``memory.py`` monolith. The class reads existing
JSONL stores and summarizes what the system has already tried: which fields
and operators worked, which hypotheses failed, and how alpha variants
relate to their parents. It does not change the primary pipeline write
path.
"""
...
from brain_alpha_ops.research.memory._helpers import (
    _as_list,
