...
``_safe_replace_token`` helper live here. They are mixed into
``IterativeOptimizer`` in ``_optimizer`` to keep the main class file under
the per-submodule line budget while preserving the public class API.
"""
...
from brain_alpha_ops.research.iterative_optimizer._types import _STRUCTURE_WRAPS
