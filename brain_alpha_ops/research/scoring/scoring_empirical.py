"""Shared utility helpers and empirical scoring from BRAIN official metrics.

Merged from the former ``_helpers`` and ``empirical`` sub-modules.  The
helpers (``_num``, ``_bounded_score``, ``item``, ``check``, etc.) are placed
here — the foundation layer — so that ``scoring_components`` and ``scoring``
