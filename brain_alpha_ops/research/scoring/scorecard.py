...
Contains ``build_scorecard`` (the top-level scoring entry point), ``submission_checklist``,
``evaluate_quality_gate`` (BRAIN Alpha Check gate), ``_scorecard_settings``, ``decision_band``,
``estimate_score_confidence``, and the schema-version constants.
"""
...
from ._helpers import _bounded_score, _format_empirical_failure, check
from .empirical import empirical_score
from .guidance import assistant_guidance_score_adjustment
from .prior import local_convergence_score, prior_score
