"""Budgeted full-market backtest planning and execution.
...
simulation scheduler (``ThreeSlotScheduler``). It supports **multi-market batch
backtests** (e.g. same expression across regions/universes for research sweeps)
and is NOT used by the production pipeline's official simulation path.
``max_workers=4`` is the ThreadPoolExecutor fan-out for multi-market batch jobs,
unrelated to ``OFFICIAL_SIMULATION_SLOT_LIMIT`` (3). The official path uses
``BacktestSlotManager`` + ``ThreeSlotScheduler``, which enforce the 3-slot
ceiling via ``assert_scheduler_consistency()``. Do NOT route official simulation
traffic through this executor.
