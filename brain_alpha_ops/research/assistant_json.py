...
def extract_json_payload(raw_output: str) -> Any:
    ...
    if raw.startswith(("{", "[")):
        try:
            ...
            last_json_error = exc
            logger.warning("invalid assistant response JSON prefix skipped: %s", exc)
    ...
    if fenced:
        try:
            ...
            last_json_error = exc
            logger.warning("invalid fenced assistant response JSON skipped: %s", exc)
    ...
    for pattern in (r"(\{.*\})", r"(\[.*\])"):
        ...
        if match:
            try:
                ...
                last_json_error = exc
                logger.warning("invalid assistant response JSON candidate skipped: %s", exc)
                continue
