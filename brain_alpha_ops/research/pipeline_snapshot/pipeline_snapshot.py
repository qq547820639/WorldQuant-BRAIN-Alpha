...
Consolidated from the original ``pipeline_snapshot.py`` monolith's split
files (``_state`` / ``_builder`` / ``_backtest_slots``). Holds the
``PipelineSnapshotServices`` and ``PipelineSnapshotState`` dataclass
containers, the ``PipelineSnapshotBuilder`` class that assembles runtime
and summary payloads, and the ``backtest_slot_snapshot`` renderer for
slot progress.
"""
