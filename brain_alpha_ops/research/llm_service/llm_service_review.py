"""Expression-review mixin and review-result data model for ``LLMService``.

Merged from the former ``_types`` (``LLMReviewResult`` only) and
``_service_review`` sub-modules.  ``LLMGenerationGuidance`` lives in
``llm_service`` to avoid a circular import.
"""
