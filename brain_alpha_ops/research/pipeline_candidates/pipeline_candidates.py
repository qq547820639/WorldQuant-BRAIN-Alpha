"""``PipelineCandidatePoolMixin`` class assembly plus local-prefilter and
pool-management mixins.

Consolidated from the original ``pipeline_candidates.py`` monolith's split
files (``_class`` / ``_local_prefilter`` / ``_pool_management``). Holds
the public ``PipelineCandidatePoolMixin`` class assembly, the
``_LocalPrefilterMixin`` (local scoring, backtest prefilter, generation-
field prefilter, local-backtest knowledge recording) plus the
module-level helpers ``_safe_float`` and ``_local_backtest_failure_category``
and the package ``logger``, and the ``_CandidatePoolManagementMixin``
(pool top-up, merge/prune, validation target/quota planning, backtest
slot selection, pending candidate bookkeeping).

The official-context and cloud-risk helper mixins live in
``pipeline_candidates_mixins.py`` and are re-assembled onto
``PipelineCandidatePoolMixin`` via multiple inheritance.
"""
...
from .pipeline_candidates_mixins import (
    _CloudRiskMixin,
