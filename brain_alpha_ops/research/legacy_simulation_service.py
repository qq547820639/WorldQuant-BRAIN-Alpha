...
Migrated from PipelineLegacySimulationMixin to standalone class
...
class LegacySimulationService:
    """Standalone legacy simulation service using composition.
    ...
    def _simulate_batch(self, candidates: list[Candidate]) -> list[Candidate]:
        ...
        for index, candidate in enumerate(candidates, start=1):
            ...
            try:
                sim_id = p.api.submit_simulation(candidate.expression, settings)
        ...
        for attempt in range(1, attempts + 1):
            for sim_id, candidate in list(running.items()):
                ...
                if status == "COMPLETED":
                    ...
                    p.officially_simulated_count += 1
                    del running[sim_id]
                    completed_count += 1
                elif status == "FAILED":
            ...
            p.services.runtime._progress(
                ...
                data={
                    "backtests": p._backtest_snapshot(candidates),
