...
describe("usePhaseState", () => {
  ...
  it("returns evaluate phase when cache-ready candidates exist", () => {
    const { result } = renderHook(() => usePhaseState({
      ...DEFAULT_INPUT,
      contextFresh: true,
      candidatesCount: 5,
  ...
  it("computes overallProgress correctly", () => {
    ...
    expect(result.current.phaseState.overallProgress).toBe(2);
  });
  ...
  it("includes badge counts in discover phase items", () => {
    const { result } = renderHook(() => usePhaseState({
      ...DEFAULT_INPUT,
      contextFresh: true,
      candidatesCount: 10,
    ...
    const candidatesItem = result.current.phaseState.phases.discover.items.find(
      (item) => item.id === "candidates"
    );
    expect(candidatesItem?.badge).toBe(10);
  });

  it("includes badge counts in evaluate phase items", () => {
    const { result } = renderHook(() => usePhaseState({
      ...DEFAULT_INPUT,
      contextFresh: true,
      candidatesCount: 10,
    ...
    const scoringItem = result.current.phaseState.phases.evaluate.items.find(
      (item) => item.id === "scoring"
    );
    expect(scoringItem?.badge).toBe(7);
  });

  it("sets connect phase status to complete when past connect phase", () => {
    const { result } = renderHook(() => usePhaseState({
      ...DEFAULT_INPUT,
      contextFresh: true,
    }));

    expect(result.current.phaseState.phases.connect.status).toBe("complete");
    expect(result.current.phaseState.phases.discover.status).toBe("active");
  });
});
