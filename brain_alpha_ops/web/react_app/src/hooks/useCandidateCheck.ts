...
export function useCandidateCheck(deps: CandidateCheckDeps) {
  ...
  const startSingleCheck = useCallback(
    async (candidate: Candidate) => {
      ...
      pipeline.setCheckingAlphaId(alphaId);
      pipeline.check.setState('loading');
      pipeline.check.setError(null);
      pipeline.check.setProgress({
      ...
      if (result?.ok) {
        ...
        pipeline.check.setProgress({
          phase: 'single_candidate_check',
          status_message: result.submittable
            ? `候选 ${alphaId} 已通过检查。`
            : `候选 ${alphaId} 检查完成，仍需处理阻断。`,
          percent_complete: 100,
        });
        notify(
          result.submittable ? 'success' : 'warning',
          result.submittable
            ? `候选 ${alphaId} 检查通过。`
            : `候选 ${alphaId} 检查完成，仍未提交就绪。`
        );
        await loadCandidates();
        onCandidatePoolUpdated?.();
      } else {
        const message = apiErrorMessage(result, '单行补查失败');
        pipeline.check.setState('error');
        pipeline.check.setError(message);
        pipeline.check.setProgress({
          phase: 'single_candidate_check',
  ...
  const startBatchCheck = useCallback(
    async (candidateOverride?: Candidate[]) => {
      const candidatesForCheck =
      ...
      if (result?.ok && nextJobId) {
        pipeline.check.setJobId(nextJobId);
  ...
  const handleCheckEvent = useCallback(
    (event: SSEEvent) => {
      try {
        ...
        pipeline.check.setProgress(progress);
        ...
        if (outcome.kind === 'failed') {
          pipeline.check.setState('error');
          pipeline.check.setError(outcome.message);
          pipeline.updateAutoPipelineStage('idle');
          notify(outcome.notifyType, outcome.message);
          pipeline.check.setJobId(null);
          return;
        }
            ...
            if (
              ...
              if (optimizationCandidatesForPool && startOptimization && generateCandidates) {
                ...
                if (autoOptimizationCycles < maxAutoOptimizationCycles && reworkCandidates.length) {
                  notify(
                    'info',
                    `主池仍缺 ${loaded.snapshot.deficit} 个候选，先优化 ${Math.min(reworkCandidates.length, autoSimulationBatchSize)} 个需优化候选。`
                  );
  ...
  const handleCheckStreamExhausted = useCallback(() => {
    ...
    void requestJobCancel({ jobId: cancelledJobId, reason: 'sse_exhausted', message }).then(
      (result) => {
        ...
        pipeline.check.setProgress((current) => ({
          ...(current || {}),
