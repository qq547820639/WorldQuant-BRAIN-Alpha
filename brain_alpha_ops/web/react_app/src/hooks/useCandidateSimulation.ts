...
export function useCandidateSimulation(deps: CandidateSimulationDeps) {
  ...
  const startSimulation = useCallback(
    async (candidate?: Candidate, candidateOverride?: Candidate[]) => {
      pipeline.simulation.setState('loading');
      pipeline.simulation.setError(null);
      const alphaId = candidate ? candidateIdentity(candidate) : '';
      pipeline.simulation.setProgress({
        phase: 'simulation_start',
        status_message: alphaId
          ? `正在启动单个候选 ${alphaId} 的官方模拟请求。`
          : '正在启动官方模拟请求。',
      });
      ...
      if (alphaId) {
        ...
        const candidatesForSimulation = hasExplicitOverride
          ...
          : workflowCandidatesForQueue(
              candidates,
              retainedPoolCandidates,
              serverWorkflowPlan?.validator?.next_candidate_ids
            );
        const cIds = simulationCandidateIds(candidatesForSimulation, AUTO_SIMULATION_BATCH_SIZE);
        if (cIds.length) {
          payload.candidate_ids = cIds;
          payload.max_simulations = Math.min(AUTO_SIMULATION_BATCH_SIZE, cIds.length);
          ...
          pipeline.simulation.setState('error');
          pipeline.simulation.setError(message);
          pipeline.simulation.setProgress({
            phase: 'simulation_start',
  ...
  const handleSimEvent = useCallback(
    (event: SSEEvent, startBatchCheck?: (candidateOverride?: Candidate[]) => Promise<void>) => {
      try {
        const progress = event.progress || event.data || {};
        pipeline.simulation.setProgress(progress);
        ...
        if (outcome.kind === 'failed') {
          pipeline.simulation.setState('error');
          pipeline.simulation.setError(outcome.message);
          pipeline.updateAutoPipelineStage('idle');
          nextBatchCheckCandidatesRef.current = null;
        ...
        if (outcome.kind === 'success') {
          ...
          if (!simulationSucceeded) {
            ...
            pipeline.simulation.setState('success');
            notify(result.failed > 0 ? 'warning' : outcome.notifyType, message);
          }
          pipeline.simulation.setJobId(null);
          if (
            pipeline.autoPipelineStageRef.current === 'await_quality_check' &&
            simulationSucceeded
          ) {
  ...
  const handleSimStreamExhausted = useCallback(() => {
    ...
    const message =
      'BRAIN模拟进度通道已耗尽，正在请求后台自动中断；若官方请求已发出，将等待当前请求返回后更新状态。';
    void requestJobCancel({
      jobId: pipeline.simulation.jobId,
