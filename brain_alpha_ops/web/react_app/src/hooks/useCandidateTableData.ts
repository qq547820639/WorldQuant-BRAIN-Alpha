...
interface UseCandidateTableDataOptions {
  ...
  refreshAll: () => void;
  callCheckResultsApi: <T = { items?: CandidateCheckResult[] }>(url: string, options?: RequestInit) => Promise<(T & { ok?: boolean; error?: string }) | null>;
  callLifecycleApi: <T = AlphaLifecycleHistoryResponse>(url: string, options?: RequestInit) => Promise<(T & { ok?: boolean; error?: string }) | null>;
...
export function useCandidateTableData(options: UseCandidateTableDataOptions) {
  ...
  const processCandidatesData = useCallback((result: typeof globalCandidatesData): LoadedCandidateState | null => {
  ...
  const loadCandidates = useCallback(async (): Promise<LoadedCandidateState | null> => {
    ...
    return loaded;
  }, [refreshAll, processCandidatesData, globalCandidatesData, callCheckResultsApi, callLifecycleApi, notify]);
