...
export function usePhaseState(input: PhaseInput) {
  ...
  const phases = useMemo<Record<PhaseId, PhaseGroup>>(
    () => ({
        ...
        items: [
          {
            ...
            label: '候选管理',
            icon: '02',
            badge: candidatesCount || undefined,
          },
      ...
      evaluate: {
        ...
        status:
          currentPhase === 'evaluate'
            ? 'active'
            : currentPhase === 'connect' || currentPhase === 'discover'
              ? 'pending'
              : 'complete',
        ...
        items: [
          {
            ...
            label: '科学评分',
            icon: '04',
            badge: scoredCount || undefined,
          },
          { id: 'official_backtests', label: '回测监控', icon: '03' },
      ...
      ready: {
        id: 'ready',
        label: '提交就绪',
        status: currentPhase === 'ready' ? (readinessPassed ? 'complete' : 'active') : 'pending',
        expanded: currentPhase === 'ready',
        unlockCondition: '通过质量门禁后进入人工审核',
        items: [{ id: 'submission_confirm', label: '阻断复核', icon: '06' }],
      },
    }),
    [
      connected,
  ...
  const phaseState: PhaseState = {
    currentPhase,
    phases,
    overallProgress: steps.filter((s) => s.status === 'complete').length,
  };
