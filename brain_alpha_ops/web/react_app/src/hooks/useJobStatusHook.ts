...
/**
 * @deprecated Use the composable useJobMonitor aggregator instead.
 */
...
export function useJobStatus(deps: JobStatusDeps): JobStatusHandlers {
  ...
  const onTerminalRecovery = useCallback(
    (result: JobStatus) => {
      ...
      if (resultState.failed || resultState.missing || resultState.interrupted) {
        const message = jobStatusMessage(
          result,
          resultState.interrupted ? '上次任务已停止，结果未确认完成。' : '任务在您离开期间失败。'
        );
        setProgressError(message);
        notify(
          resultState.interrupted ? 'warning' : 'error',
          resultState.interrupted ? '上次任务已停止。' : '上次任务已失败。'
        );
      } else {
        notify('info', '上次任务已完成。');
      }
  ...
  const onSseTerminal = useCallback(
    (event: SSEEvent, eventOutcome: ReturnType<typeof resolveJobEventState>) => {
      clearSavedJobId();
      setRunning(false);
      setPollFailures(0);
      setJobId(null);
  ...
  const onSseProgress = useCallback((event: SSEEvent) => {
    saveResumeState({
      totalCyclesCompleted:
        typeof event.progress?.current === 'number' && event.progress.current > 0
  ...
  const onSseCandidate = useCallback(
    (alphaId: string) => {
  ...
  const onSseSubmission = useCallback(
    (alphaId: string) => {
