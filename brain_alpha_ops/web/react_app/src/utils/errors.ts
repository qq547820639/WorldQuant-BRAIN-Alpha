 ...
 *
 * Merged from former `errorHandler.ts` (ApiError/ValidationError classes +
 * logging/formatting helpers) and `reportIgnoredError.ts` (dev-only ignored
 * error logger).
 */
...
// ── Ignored-error logger (former reportIgnoredError.ts) ───────────────────
