...
describe('usePhaseState', () => {
  ...
  it('keeps loading phase state neutral while backend phase state is unknown', () => {
    ...
    expect(result.current.phaseState.phases.connect.unlockCondition).toContain(
      '读取完成前不判定为未连接'
    );
  ...
  it('keeps phase read errors separate from disconnected setup', () => {
    ...
    expect(result.current.phaseState.phases.connect.unlockCondition).not.toContain(
      '连接 BRAIN 账户'
    );
