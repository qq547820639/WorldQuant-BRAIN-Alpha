...
export const RAW_UNSAFE_DISPLAY_TEXT_PATTERN =
  /(?:raw\s+backend|raw_backend|RAW_BACKEND|SESSION_INVALID|session_invalid|invalid local session|traceback|exception|stack trace|csrf[_-]?token|session[_-]?id|access[_-]?token|refresh[_-]?token|api[_-]?key|client[_-]?secret|password|passwd|pwd|token=|password=|api_key=|csrf_token=)/i;

const SESSION_INVALID_VALUES = new Set([
  'session_invalid',
  'invalid local session',
  'session_expired',
]);

export function userErrorFromPayload(
  payload: ApiErrorExperiencePayload | null | undefined
): ApiUserError | null {
  if (!payload || typeof payload.user_error !== 'object' || payload.user_error === null)
    return null;
...
export function safeDisplayErrorMessage(
  value: unknown,
  fallback = '请求失败，请稍后重试。'
): string {
...
export function knownApiErrorMessage(value: unknown): string | null {
  ...
  if (
    normalized === 'job_not_found' ||
    normalized === 'unknown job' ||
    normalized === 'unknown sync job'
  ) {
  ...
  if (
    normalized === 'concurrent_simulation_limit_exceeded' ||
    normalized.includes('concurrent simulation limit')
  ) {
  ...
  if (
    normalized === 'official_context_refresh_timeout' ||
    normalized === 'official context timeout'
  ) {
...
function unsafeBackendText(text: string): boolean {
  ...
  if (
    /(?:raw\s+backend|raw_backend|traceback|stack trace|session_invalid|invalid local session)/i.test(
      text
    )
  ) {
    return true;
  ...
  if (
    /(?:password|passwd|pwd|token|csrf(?:[-_\s]?token)?|cookie|set[-_\s]?cookie|authorization|api[-_\s]?key|client[-_\s]?secret|access[-_\s]?token|refresh[-_\s]?token|id[-_\s]?token|session(?:[-_\s]?(?:id|key|token))?)\s*[:=]\s*[A-Za-z0-9._~+\/=-]{8,}/i.test(
      text
    )
  ) {
    return true;
