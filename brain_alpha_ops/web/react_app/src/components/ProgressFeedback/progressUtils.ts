...
export function normalizedPercent(progress?: UnifiedProgress | null, classification?: JobStateClassification): number | null {
...
export function etaSecondsFromProgress(progress?: UnifiedProgress | null, options?: { deadlineOnly?: boolean }) {
...
export function estimatedEtaSeconds(progress: UnifiedProgress | null | undefined, elapsed: number, fallback: number) {
  const scanned = positiveNumber(progress?.scanned);
...
export function displayProgressPhase(progress: UnifiedProgress | null | undefined, fallback: string) {
