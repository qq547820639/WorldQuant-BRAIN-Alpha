    ...
    <div className="panel mb-4">
      ...
      <div className="panel-body-padded">
        {loading ? (
          ...
          <ProgressFeedback state="error" title="云端 Alpha" error={error} onRetry={onRetry} compact />
        ) : cloud ? (
          <>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs mb-3">
              <div><span className="text-text-tertiary">缓存总数</span><p className="font-mono-value text-base text-text-primary">{summary.count ?? 0}</p></div>
              <div><span className="text-text-tertiary">已提交</span><p className="font-mono-value text-base text-positive">{summary.submitted_count ?? 0}</p></div>
              <div><span className="text-text-tertiary">已通过（未提交）</span><p className="font-mono-value text-base text-warning">{summary.passed_unsubmitted_count ?? 0}</p></div>
              <div>
                <span className="text-text-tertiary">缓存状态</span>
                <p className={`text-sm ${summary.is_stale ? "font-medium text-warning" : "text-text-secondary"}`}>
                  {summary.is_stale ? "已过期" : "有效"}
                </p>
                {summary.is_stale && (
                  <button type="button" className="mt-1 text-xs text-accent underline" onClick={onOpenSync}>
                    去手动同步
                  </button>
                )}
              </div>
