...
export default function CandidateTableMobile({
  ...
  return (
    <div ref={mobileContainerRef} className="panel-body md:hidden overflow-auto" style={{ height: "min(640px, 70vh)" }}>
      {candidates.length === 0 ? (
        <EmptyState
          ...
          {mobileVirtualizer.getVirtualItems().map((virtualItem) => {
            ...
            return (
              <CandidateMobileCard
                ...
                onCheck={onCheck}
                cardRef={(node) => { if (node) mobileVirtualizer.measureElement(node); }}
