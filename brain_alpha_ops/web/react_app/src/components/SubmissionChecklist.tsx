...
function candidateIds(
  row: Pick<Candidate, 'alpha_id' | 'official_alpha_id' | 'simulation_id'> | CheckResult
) {
  ...
  return (
    <section className="min-w-0 space-y-3">
      ...
      <div className="space-y-3 md:hidden" aria-label={`${title} 移动端卡片`}>
        {rows.length === 0 ? (
          <div className="rounded-md border border-border-subtle bg-[var(--color-surface-elevated)] p-4 text-center text-sm text-text-tertiary">
            {empty}
          </div>
        ) : (
          rows.map((row) => (
            <article
              key={`${title}_mobile_${row.id}`}
              className="rounded-md border border-border-subtle bg-[var(--color-surface-elevated)] p-4 text-sm"
            >
              <div className="flex min-w-0 items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="break-all font-mono-value text-xs text-accent">{row.id}</p>
                  <p className="mt-1 break-words font-mono-value text-xs text-text-secondary">
                    {row.expression || '-'}
                  </p>
                </div>
                <span
                  className={`badge shrink-0 text-xs ${row.status === 'READY' ? 'badge-positive' : 'badge-warning'}`}
                >
              ...
              {rows.length === 0 ? (
                ...
                rows.map((row) => (
                  <tr key={`${title}_${row.id}`} className="border-b border-border-subtle">
                    ...
                    <td
                      className="max-w-xs truncate p-3 font-mono-value text-xs"
                    ...
                    <td className="p-3">
                      <span
                        className={`badge text-xs ${row.status === 'READY' ? 'badge-positive' : 'badge-warning'}`}
                      >
                        {readinessStatusLabel(row.status)}
                      </span>
                    </td>
                    ...
                    <td
                      className="max-w-sm break-words p-3 text-xs text-text-secondary"
                      title={row.reasons}
                    >
                      {row.reasons || '-'}
                    </td>
