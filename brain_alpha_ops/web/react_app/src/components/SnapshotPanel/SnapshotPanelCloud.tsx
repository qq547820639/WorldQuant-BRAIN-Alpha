import { text, record, metricText, compactJoin, rowId, type SnapshotPayload, type SnapshotRow } from "./utils";
...
export function cloudMetrics(payload: SnapshotPayload, rows: SnapshotRow[]) {
  ...
  return [
    { label: "缓存总数", value: text((summary.total ?? summary.count ?? summary.total_count ?? "-") as string) },
    { label: "载入状态", value: cloudLoadStatus(summary) },
    { label: "已提交", value: text((summary.submitted_count ?? "-") as string) },
    { label: "已通过", value: text((summary.passed_unsubmitted_count ?? "-") as string) },
  ];
}
