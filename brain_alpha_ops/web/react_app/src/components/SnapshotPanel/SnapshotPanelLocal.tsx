import {
  text, record, array, truthy, metricText, safeSnapshotDisplayText,
  rowId, dedupeSnapshotRows, namedRows, signalRows, compactJoin,
...
export function checkpointStatusRows(payload: SnapshotPayload): SnapshotRow[] {
  ...
  rows.push(...(payload.checkpoints as unknown[] || []).map((item, index) => {
  ...
  rows.push(...(payload.history as unknown[] || []).map((item, index) => {
    const row = record(item);
    return {
