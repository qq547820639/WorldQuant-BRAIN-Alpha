  ...
  const layers = [
    {
      ...
      weight: lw.prior ?? 0.3,
      contribution: (scorecard.prior?.score ?? scorecard.prior_score ?? 0) * (lw.prior ?? 0.3),
    ...
    },
    {
      ...
      contribution:
        (scorecard.empirical?.score ?? scorecard.empirical_score ?? 0) * (lw.empirical ?? 0.45),
      ...
      {layers.map((layer) => (
        ...
        >
          <div className="scorebreakdown-label" style={{ fontWeight: 600 }}>
            ...
            <span className="scorebreakdown-weight">× {`${(layer.weight * 100).toFixed(0)}%`}</span>
          </div>
  ...
  return (
    <>
      <div className="scorebreakdown-header">
        ...
        <span className="scorebreakdown-total-value">{formatScore(scorecard.total_score)}</span>
        <span
          className={`badge ${
          ...
          <div className="scorebreakdown-gates">
            ...
            <div className="scorebreakdown-gates-list">
              {hardGates.map((gate) => (
                ...
                >
                  <span className="scorebreakdown-gate-icon">{gate.passed ? '✓' : '✗'}</span>
                  <span className="scorebreakdown-gate-name">{labelFor(gate.name)}</span>
                </div>
              ))}
            </div>
          </div>
