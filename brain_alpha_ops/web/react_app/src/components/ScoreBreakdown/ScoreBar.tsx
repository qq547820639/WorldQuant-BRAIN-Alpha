...
export default function ScoreBar({ node, depth, maxScore }: Props) {
  ...
  return (
    <>
      ...
      >
        <div className="scorebreakdown-label" style={{ fontWeight: isLayer ? 600 : 400 }}>
          <span style={{ color }}>
            {isLayer ? (LAYER_NAMES[node.name] ?? labelFor(node.name)) : labelFor(node.name)}
          </span>
          {isLayer && <span className="scorebreakdown-weight">× {formatPercent(node.weight)}</span>}
        </div>

        <div className="scorebreakdown-values">
          {isLayer || isLeaf ? (
            <>
              <span className="scorebreakdown-score" style={{ color }}>
                {formatScore(node.score)}
              </span>
              {isLayer && (
                <>
                  <span className="scorebreakdown-contrib-label">→</span>
                  <span className="scorebreakdown-contrib" style={{ color }}>
              ...
              {isLeaf && depth === 2 && node.name !== 'total_score' && (
                <>
                  <span className="scorebreakdown-contrib-label">×</span>
                  <span className="scorebreakdown-contrib-minor">{formatPercent(node.weight)}</span>
                  <span className="scorebreakdown-contrib-label">→</span>
