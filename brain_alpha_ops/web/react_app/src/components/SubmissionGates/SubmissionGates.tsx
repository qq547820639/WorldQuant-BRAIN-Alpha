...
export function ReadinessSummary({
  ...
  const hasBlockers =
    allBlockers.length > 0 || allFamilyBlockers.length > 0 || allProductionGaps.length > 0;
      ...
      <section className="rounded-md border border-border-subtle bg-[var(--color-surface-elevated)] px-3 py-3">
        <dl className="grid grid-cols-2 gap-3 text-xs sm:grid-cols-4">
          <ReadinessMetric
            label="阻断复核"
            value={ready ? '通过' : '未通过'}
            tone={ready ? 'text-positive' : 'text-warning'}
          />
          ...
          <ReadinessMetric
            label="官方接口"
            value={readiness?.official_api_called ? '已调用' : '未调用'}
          />
          <ReadinessMetric label="最佳 Alpha" value={best.alpha_id || '-'} mono />
          <ReadinessMetric
            label="真实提交"
            value={readiness?.real_submit_performed ? '真实提交已发生' : '未执行真实提交'}
            tone={readiness?.real_submit_performed ? 'text-negative' : 'text-positive'}
          />
        </dl>
        <div className="mt-3 space-y-1 text-xs text-text-tertiary">
          ...
          <p className="break-words">
            ...
            {readiness?.authoritative_stop_rule ||
              readiness?.validation_command ||
              readiness?.source ||
              'check_live_submit_readiness.py'}
          </p>
        </div>
      </section>
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-1 text-xs text-text-secondary leading-relaxed">
                您的候选已通过全部质量门禁。Web 控制台仅负责就绪复核，不执行真实提交。 请前往 BRAIN
                平台控制台完成最终提交。
              </p>
        ...
        <section className="rounded-md border border-[var(--color-blocked-section-border)] bg-[var(--color-blocked-section-bg)] p-4">
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0 flex-1">
              <h3 className="text-sm font-semibold text-text-primary">
                提交前仍有{' '}
                {allBlockers.length + allFamilyBlockers.length + allProductionGaps.length}{' '}
                项阻断需要处理
              </h3>
              <p className="mt-1 text-xs text-text-secondary leading-relaxed">
      ...
      {hasBestCandidate && !showManualSubmitGuidance && (
        <section className="rounded-md border border-border-subtle bg-[var(--color-surface-elevated)] px-3 py-3 text-xs text-text-tertiary">
          ...
          <div className="mt-1 space-y-0.5">
            <p>
              Alpha: <span className="font-mono-value text-accent">{best.alpha_id || '-'}</span>
            </p>
            <p>决策: {readinessReasonLabel(best.decision_band || '')}</p>
            <p>分数: {best.score != null ? Number(best.score).toFixed(2) : '-'}</p>
            <p>
              相似度: {best.max_similarity != null ? Number(best.max_similarity).toFixed(4) : '-'}
            </p>
