interface TaskSuccessBannerData {
  newCount: number;
  optimizedCount: number;
}

interface Props {
  banner: TaskSuccessBannerData | null;
  retainedPoolCount: number;
  targetPoolSize: number;
  onClose: () => void;
}

export default function TaskSuccessBanner({
  banner,
  retainedPoolCount,
  targetPoolSize,
  onClose,
    ...
    >
      ...
      >
        ...
        <span className="text-xs text-text-secondary">
          新增{' '}
          <span className="font-mono-value text-positive">{banner.newCount}</span> 个候选
        </span>
        {banner.optimizedCount > 0 && (
          <span className="text-xs text-text-secondary">
            优化{' '}
            <span className="font-mono-value text-accent">{banner.optimizedCount}</span> 个
          </span>
        )}
        <span className="text-xs text-text-tertiary">
          当前池状态：
          <span className="font-mono-value text-text-primary">
            {retainedPoolCount}/{targetPoolSize}
          </span>
        </span>
      </div>
      <button
...
export type { TaskSuccessBannerData };
