...
export function generateReportMarkdown(input: ReportInput): string {
  const { trendCandidates, trendSubmissions, cloudSummary, memory, status, cloudPreviewRows } =
    input;
  const now = new Date();
  ...
  lines.push(
    `| 当前候选池 | ${memory?.total_candidates ?? status?.progress?.candidates_generated ?? 0} |`
  );
  ...
  lines.push('---');
  lines.push(
    `*由 BRAIN-Alpha 本地控制台自动生成 · ${now.toISOString().slice(0, 19).replace('T', ' ')}*`
  );
...
export const DashboardReportModal = memo(function DashboardReportModal({
  show,
  onClose,
  markdown,
  ...
  return (
    ...
    >
      <div
        ref={dialogRef}
        className="flex flex-col rounded-lg shadow-lg border border-modal max-w-[640px] w-[calc(100%-32px)] max-h-[80vh]"
      ...
      >
        <div className="flex justify-between items-center px-5 py-4 border-b border-modal-border">
          <h2 id="report-modal-title" className="text-base font-semibold text-text-primary">
            📋 工作日报
          </h2>
          <button
            ref={closeButtonRef}
        ...
        <div
          className="flex-1 overflow-y-auto p-5 font-mono text-[13px] leading-relaxed whitespace-pre-wrap border-y border-divider"
          style={{ color: 'var(--color-report-text)', backgroundColor: 'var(--color-report-bg)' }}
        >
          {markdown}
        </div>
