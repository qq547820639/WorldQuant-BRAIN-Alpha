"""Backtest slot payload, queue-summary route handlers, and classification helpers.

Consolidated from the former ``web_backtest_slots/`` subpackage (handlers
module) plus its ``web_backtest_slots_helpers`` sidecar. Builds the per-slot
payload, status board, and local-readonly queue summary served by the Web
routes, and provides the official-review blocker classification, slot/row
predicates, and queue-summary helpers used by those handlers.

The legacy flat import ``brain_alpha_ops.web_backtest_slots`` (redirected by
``_web_bridge``) continues to resolve to this module, so existing callers
