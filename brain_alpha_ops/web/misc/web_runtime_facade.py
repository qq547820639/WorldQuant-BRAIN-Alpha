...
(``_logging`` + ``_dispatch_context`` + ``_server`` modules) and the former
``web_runtime_facade_services.py`` (job dispatch, snapshot factories, and
submission/check delegates). These helpers are bound to the public
``brain_alpha_ops.web`` facade at runtime and delegate to service-layer
functions via ``web.*`` attributes.
...
# ═══════════════════════ Job dispatch, snapshot, and submission delegates ═══
# Consolidated from web_runtime_facade_services.py. These helpers are bound
# to the public ``brain_alpha_ops.web`` facade at runtime and delegate to
# service-layer functions via ``web.*`` attributes.
