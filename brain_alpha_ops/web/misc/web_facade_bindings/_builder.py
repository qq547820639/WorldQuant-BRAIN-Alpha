"""Builder for the web facade bindings namespace.

Houses ``build_web_facade_bindings`` which assembles the runtime namespace
dictionary consumed by ``brain_alpha_ops.web``. Extracted from the former
``web_facade_bindings.py`` monolith (deep-optimization-phase12, Task B8).
The bound helpers are re-exported by the sibling ``_*_imports`` submodules.
"""
