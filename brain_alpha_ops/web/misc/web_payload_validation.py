"""Request payload validation helpers and server lifecycle helpers for the local web console.

Consolidated from the former ``web_payload_validation.py`` (request payload
validators + constants) and ``web_server_lifecycle.py`` (``ThreadingHTTPServer``
subclass, free-port finder, ``serve`` / ``shutdown_server`` /
``smoke_test_server``). The payload section enforces bounds for every local web
route body; the server-lifecycle section hosts the canonical server factory and
bind/shutdown helpers used by the CLI entrypoint and the smoke test.
"""
