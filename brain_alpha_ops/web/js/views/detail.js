...
(function () {
  ...
  function renderAttributionTreeHTML(result) {
    ...
    body += '<div style="font-weight:700;font-size:13px;margin-bottom:4px;color:var(--text)">🔒 硬门禁</div>';
    if (hg.length) {
      hg.forEach(function (g) {
        var color = g.passed ? 'var(--good)' : 'var(--bad)';
        var icon = g.passed ? '✓' : '✗';
        body += '<div style="font-size:12px;padding:2px 0;color:' + color + '">' + icon + ' ' + g.gate_name;
        if (g.failed_items && g.failed_items.length) {
          body += ' <span style="color:var(--muted)">(' + g.failed_items.join(', ') + ')</span>';
        }
        body += '</div>';
      });
    } else {
    ...
    if (sg.length) {
      body += '<div class="detail-section" style="margin-top:6px">';
      body += '<div style="font-weight:700;font-size:13px;margin-bottom:4px;color:var(--text)">⚠ 软门禁警告</div>';
      sg.forEach(function (g) {
        var color = g.passed ? 'var(--muted)' : 'var(--warn)';
        body += '<div style="font-size:12px;padding:2px 0;color:' + color + '">' + g.gate_name;
        if (g.failed_items && g.failed_items.length) {
          body += ' (' + g.failed_items.join(', ') + ')';
        }
        body += '</div>';
      });
      body += '</div>';
    ...
    if (tf.length) {
      body += '<div class="detail-section" style="margin-top:6px">';
      body += '<div style="font-weight:700;font-size:13px;margin-bottom:4px;color:var(--bad)">✗ 主要失败项</div>';
      tf.forEach(function (f) {
        body += '<div style="font-size:12px;padding:2px 0;color:var(--bad)">' + f.name + ': ' + (f.reason || f.actual || '') + '</div>';
      });
      body += '</div>';
    }
    ...
    if (hints.length) {
      body += '<div class="detail-section" style="margin-top:6px">';
      body += '<div style="font-weight:700;font-size:13px;margin-bottom:4px;color:var(--text)">💡 改进建议</div>';
      hints.forEach(function (h) {
        body += '<div style="font-size:12px;padding:2px 0;color:var(--muted)">' + h + '</div>';
      });
      body += '</div>';
    }
    ...
    if (result.attribution) {
      body += '<div class="detail-section" style="margin-top:6px">';
      body += '<div style="font-weight:700;font-size:13px;margin-bottom:4px;color:var(--text)">🌳 评分归因树</div>';
