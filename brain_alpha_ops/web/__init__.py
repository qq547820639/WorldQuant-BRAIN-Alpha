...
def _run_submit_alpha_job(job_id: str, payload: dict) -> None:
    ...
    try:
        ...
        _job_update(job_id, progress={
            ...
            "status_message": f"正在认证并提交模拟: {expression}...",
            "percent_complete": 10,
        })
        api.authenticate()
        ...
        _job_update(job_id, progress={
            "phase": "submission", "status": "running",
            "status_message": f"正在提交 BRAIN 模拟...",
            "percent_complete": 15,
...
def dispatch_post(handler, path, body):
    ...
    if fn:
        result = fn(payload)
        if path in ("/api/test_connection", "/api/connection_test") and result.get("ok") and result.get("connected"):
            from brain_alpha_ops.web_session import mark_brain_connection_verified
            mark_brain_connection_verified(handler._session_id_from_cookie(), result, payload)
        if hasattr(handler, "_send_json"):
