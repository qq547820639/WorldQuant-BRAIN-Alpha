...
本文档面向最终用户，重点说明如何从零启动、如何登录真实 BRAIN 生产环境、如何完成核心功能走查，以及遇到问题时如何判断下一步。文档内项目截图均来自真实生产环境走查，截图区域已避开账号、密码、Token 等凭据输入位置，不包含 Mock 数据、空状态截图或演示数据截图。

## 1. 本次真实生产走查结论

本次 README 证据在 2026-05-29 08:51 CST 通过本地 Web 控制台完成复核：
...
| 截图数据 | Alpha ID、状态、Sharpe、Fitness、Turnover、检查结论、失败原因均来自真实生产数据 |
| 提交动作 | 未勾选提交项，未触发真实提交 |
| 凭据处理 | 凭据仅用于本次登录，不写入 README、配置文件或截图 |

本次也实际点击了“同步云端数据”。官方远端在扫描到 15,600 条记录后断开连接，任务被记录为可重试的网络失败：`Remote end closed connection without response`。因此本文不会把该次同步伪装为成功；截图展示的是已连接生产账号后加载到页面中的真实生产云端快照和详情数据。
...
生产走查中，页面连接状态显示为 `已连接 — production · session_cookie`。截图前已清空可见账号和密码输入框。
...
本次生产页面显示了非空云端数据表，首屏包含 `9qaOZVno`、`6XaQMQdP`、`vRebAPaw`、`zqPEEEjR` 等真实 Alpha 记录，且包含 Sharpe、Fitness、Turnover 指标。
...
本次生产走查打开了 `9qaOZVno` 的详情。该记录状态为 `UNSUBMITTED`，Sharpe 为 `1.9200`，Fitness 为 `2.1500`，Turnover 为 `0.0313`，检查结论为 `FAIL`，失败原因为 `LOW_SUB_UNIVERSE_SHARPE`。
...
如果官方接口临时断开，页面会记录失败原因。可等待几分钟后重试，或先使用已加载的云端快照查看数据。本次走查中同步任务扫描到 15,600 条后遇到官方远端断开，属于可重试网络失败。
...
本次 README 生产走查没有勾选提交项，也没有触发真实提交。

## 5. 只读验证模式

如果你只想确认项目能读取真实生产数据，不希望触发候选生产或提交，可以按以下只读路径走查：
