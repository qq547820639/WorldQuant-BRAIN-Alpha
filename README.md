# BRAIN Alpha Ops

BRAIN Alpha Ops 是一个面向 WorldQuant BRAIN 的本地 Alpha 研究与生产辅助工具。它把候选 Alpha 生成、本地评分、官方 Check、官方 Simulation、提交前检查、运行历史和诊断门禁组织成一条可审计的工作流。

项目的默认目标不是绕过 BRAIN 平台规则，而是把所有关键动作尽量对齐官方 API、官方字段、官方算子、官方数据集和可追踪配置，减少手工试错和不安全提交。

## 当前状态

最近一次本地源代码验证时间：2026-05-26。

| 项目 | 当前结果 |
|---|---|
| 生产诊断报告 | PASS |
| 六大技术红线 | 73/73 passed |
| 官方上下文 | fields=7780, operators=66, datasets=17 |
| 官方上下文 freshness | strict mode passed |
| 参数审计 | thresholds_zero_deviation=True, trace sections=6 |
| 诊断 Gap 覆盖 | P0=0, P1=0 |
| 全量测试 | 755 passed, 1 existing pytest config warning |

源代码层面当前没有 P0/P1 本地代码清单项。仍然建议把最终发布判断和打包后的 `dist\BrainAlphaOps.exe` 启动验证分开处理，因为 EXE 是否真正可启动是独立的发布门槛。

## 能做什么

- 生成候选 Alpha：支持假说驱动、经验反馈和随机探索等生成路径。
- 本地预筛选：用评分、去重、表达式检查、数据合规和风险门槛减少无效官方调用。
- 官方 API 对接：调用 WorldQuant BRAIN 官方接口执行认证、字段/算子/数据集刷新、Alpha Check、Simulation 和提交。
- 安全提交：默认关闭自动提交，提交前执行官方 Check、相似度、质量门槛和提交策略约束。
- Web 控制台：在本地浏览器查看候选池、回测队列、运行状态、云端 Alpha、历史记录和诊断结果。
- 运行历史：保存每次运行的结果、参数审计快照、checkpoint 和统计对比。
- 质量门禁：把配置校验、红线检查、官方上下文 freshness、诊断 Gap、前端同步、敏感信息扫描和测试串起来。

## 环境要求

- Windows PowerShell
- Python 3.10 或更高版本
- WorldQuant BRAIN 账号
- 可选：PyInstaller，用于打包 Windows EXE

Web 控制台服务端使用 Python 标准库实现，不需要额外前端构建链路。

## 安装

```powershell
git clone https://github.com/qq547820639/WorldQuant-BRAIN-Alpha.git
cd WorldQuant-BRAIN-Alpha

python -m venv venv
.\venv\Scripts\Activate.ps1

pip install -e .
```

开发和测试环境：

```powershell
pip install -e ".[test,dev]"
```

如果你在 Codex 桌面环境中运行，本仓库常用的稳定方式是使用内置 Python，并让 `PYTHONPATH` 包含 `.codex_pydeps`：

```powershell
$env:PYTHONPATH = ".codex_pydeps"
& "C:\Users\54782\.cache\codex-runtimes\codex-primary-runtime\dependencies\python\python.exe" -m pytest -q --basetemp .pytest_tmp
```

## 凭据设置

不要把 BRAIN 用户名、密码或 token 写进代码、配置文件、README、日志或 commit。运行时用进程环境变量传入：

```powershell
$env:BRAIN_USERNAME = "your@email.com"
$env:BRAIN_PASSWORD = "your_password"
```

如果使用 token：

```powershell
$env:BRAIN_TOKEN = "your_token"
```

配置文件 [config/run_config.json](config/run_config.json) 只保存环境变量名称，不保存真实凭据。

## 快速开始

先校验配置：

```powershell
python -m brain_alpha_ops.cli validate-config --config config/run_config.json
```

刷新官方字段、算子和数据集上下文：

```powershell
python fetch_official_context.py --config config/run_config.json --json
```

运行一轮研究流水线：

```powershell
python -m brain_alpha_ops.cli run --config config/run_config.json --cycles 1 --candidates 20
```

运行带 checkpoint 和进度阶段的引导模式：

```powershell
python -m brain_alpha_ops.cli guided-run --config config/run_config.json
```

启动本地 Web 控制台：

```powershell
python launch_web.py
```

默认地址：

```text
http://127.0.0.1:8765
```

## Web 控制台

Web 控制台适合日常操作和排查。它覆盖候选生成、云端同步、批量 Check、Simulation 状态、提交前检查、运行历史和诊断视图。

常见视图：

| 视图 | 用途 |
|---|---|
| 候选池 | 查看本轮生成和保留的 Alpha 候选 |
| 待回测 | 查看通过本地门槛、准备进入官方 Simulation 的候选 |
| 回测中 | 查看正在等待官方 Simulation 结果的任务 |
| 达标 | 查看通过质量门槛的候选 |
| 可提交 | 查看满足提交前置条件的候选 |
| 已提交 | 查看已提交到 BRAIN 的记录 |
| 不达标 | 查看失败候选和原因 |
| 云端数据 | 同步和去重 BRAIN 平台已有 Alpha |
| 研究记忆 | 查看历史运行摘要、经验反馈和对比 |
| 诊断 | 查看红线、参数、数据链路、评分和 UX 覆盖状态 |

Web 控制台默认只监听本机 `127.0.0.1`。如果需要远程访问，先审查 [config/run_config.json](config/run_config.json) 中的 `web.allow_remote` 和管理 token 设置。

## 常用 CLI

所有命令都通过 `python -m brain_alpha_ops.cli <command>` 调用。

| 命令 | 用途 |
|---|---|
| `run` | 运行完整研究流水线 |
| `guided-run` | 运行带 checkpoint、阶段进度和历史记录的引导流程 |
| `validate-config` | 校验配置文件 |
| `init-config` | 生成默认配置 |
| `diagnose` | 生成生产诊断、Gap 矩阵和升级建议 |
| `score` | 对单个候选 Alpha 执行完整评分 |
| `redline` | 执行六大技术红线检查 |
| `release-gate` | 执行最终发布就绪检查 |
| `memory-summary` | 汇总本地研究记忆 |
| `memory-guidance` | 导出生成器可用的研究记忆指导 |
| `expression-index` | 查询表达式历史索引 |
| `record-index` | 查询云端 Alpha/backtest 记录索引 |
| `research-observability` | 汇总研究健康状态、回测、错误和日志 |
| `assistant-context` | 导出 LLM 可用上下文包 |
| `assistant-request` | 生成 provider-neutral 的 LLM 请求包 |
| `assistant-parse` | 解析并规范化 LLM JSON 响应 |
| `assistant-guidance` | 把 LLM 响应转换成生成器指导 |
| `assistant-save-guidance` | 保存可复用 LLM 指导 |
| `assistant-guidance-audit` | 审计已保存指导的可复用性 |
| `assistant-cross-review` | 对 LLM 响应做交叉审查 |
| `anti-overfit` | 对候选执行反过拟合检查 |
| `rolling-validate` | 对候选执行滚动窗口验证 |

示例：

```powershell
python -m brain_alpha_ops.cli diagnose --config config/run_config.json --json --output docs/ALPHA_PRODUCTION_DIAGNOSIS_20260522.md
python -m brain_alpha_ops.cli redline --json
python -m brain_alpha_ops.cli release-gate --config config/run_config.json --json
```

## 配置重点

主配置文件是 [config/run_config.json](config/run_config.json)。

| 配置区 | 说明 |
|---|---|
| `environment` | 当前只支持 `production` |
| `auto_submit` | 是否允许自动提交，默认 `false` |
| `credentials` | 凭据环境变量名，不保存真实凭据 |
| `web` | 本地控制台 host、port、浏览器打开策略和管理 token |
| `ops.settings` | BRAIN 平台设置，如 region、universe、delay、neutralization、truncation |
| `ops.budget` | 每轮候选数、官方 Check 数、Simulation 数、并发数和 retained pool |
| `ops.thresholds` | Sharpe、Fitness、Turnover、Correlation、Concentration 等硬门槛 |
| `ops.submission_policy` | 每日提交上限、相似度上限、微变体阻断和提交前 Check |
| `ops.official_api` | 官方 API path、超时、轮询、速率限制和缓存 TTL |

默认关键阈值：

| 阈值 | 默认值 |
|---|---|
| `min_sharpe` | 1.25 |
| `min_sharpe_delay0` | 2.0 |
| `min_fitness` | 1.0 |
| `min_fitness_delay0` | 1.3 |
| `min_turnover` | 0.01 |
| `platform_max_turnover` | 0.70 |
| `max_self_correlation` | 0.70 |
| `max_prod_correlation` | 0.70 |
| `max_weight_concentration` | 0.10 |
| `sub_universe_sharpe_min_ratio` | 0.75 |

## 质量门禁

快速门禁：

```powershell
python scripts/quality_gate.py --skip-tests --json
```

严格官方上下文门禁：

```powershell
python scripts/quality_gate.py --strict-official-context --skip-tests --json
```

完整门禁和测试：

```powershell
python scripts/quality_gate.py --strict-official-context --json
```

常用单项检查：

```powershell
python -m compileall -q brain_alpha_ops scripts tests
python scripts/check_official_context.py --config config/run_config.json --strict-freshness --json
python scripts/check_brain_contract.py --config config/run_config.json --strict-freshness --json
python scripts/check_diagnosis_gap_coverage.py --config config/run_config.json --strict-freshness --json
python scripts/check_diagnostic_report.py --config config/run_config.json --report docs/ALPHA_PRODUCTION_DIAGNOSIS_20260522.md --json
python scripts/scan_sensitive_artifacts.py --root . --json --fail-on-findings
python scripts/check_module_size.py --json
```

全量测试：

```powershell
pytest -q --basetemp .pytest_tmp
```

在 Codex 桌面环境中：

```powershell
$env:PYTHONPATH = ".codex_pydeps"
& "C:\Users\54782\.cache\codex-runtimes\codex-primary-runtime\dependencies\python\python.exe" -m pytest -q -p no:cacheprovider --basetemp .pytest_tmp
```

## 生产诊断报告

生成最新诊断报告：

```powershell
python -m brain_alpha_ops.cli diagnose --config config/run_config.json --json --output docs/ALPHA_PRODUCTION_DIAGNOSIS_20260522.md
```

报告会覆盖：

- 六大技术红线
- 官方字段、算子、数据集加载和 freshness
- 参数审计和阈值零偏差
- BRAIN contract 对齐
- 本地评分、API-shaped simulation 和归因摘要
- Web 前端 inline 同步
- checkpoint 和 run-history replay
- 诊断 Gap 矩阵
- 完成项和未完成项

## Windows EXE 打包

打包前先跑门禁：

```powershell
python scripts/quality_gate.py --strict-official-context --skip-tests --json
```

构建 EXE：

```powershell
.\scripts\build_windows.ps1
```

输出位置：

```text
dist\BrainAlphaOps.exe
```

打包脚本会把以下发布所需资源复制到 `dist`：

- `data\official_fields.json`
- `data\official_operators.json`
- `data\official_datasets.json`
- 对应 `.meta.json`
- `data\official_context_refresh_status.json`
- `brain_alpha_ops\research\hypotheses`
- `brain_alpha_ops\research\prompts`

打包后的发布验证建议：

```powershell
.\dist\BrainAlphaOps.exe --smoke-test --port 8765
.\dist\BrainAlphaOps.exe --no-browser --port 8765
Invoke-WebRequest -UseBasicParsing http://127.0.0.1:8765/
Invoke-WebRequest -UseBasicParsing http://127.0.0.1:8765/api/health
```

发布判断不要只看源码测试。最终仍要确认打包后的 EXE 能启动、能加载配置、能加载官方上下文和假说库、Web 首页和 `/api/health` 都可访问。

## 项目结构

```text
brain_alpha_ops/
  brain_api/                  官方 BRAIN API 封装和请求适配
  compliance/                 六大技术红线和合规校验
  data/                       官方上下文、cache metadata 和数据校验
  research/                   Alpha 生成、pipeline、记忆、回测协调和策略
  scoring/                    官方对齐评分、门槛和归因
  ux/                         guided-run、checkpoint 和历史分析
  web/                        Web 静态资源，HTML/CSS/JS
  web_*.py                    Web 后端功能模块
  cli.py                      CLI 入口
  config.py                   配置模型、加载和校验
  production_diagnostics.py   生产诊断和 Gap 矩阵
  parameter_audit.py          运行参数审计快照
  diagnosis_gap_coverage.py   诊断方案可执行覆盖检查

config/
  run_config.json             主配置
  presets.json                策略预设

data/
  official_*.json             官方字段、算子、数据集缓存
  official_*.meta.json        官方上下文 metadata
  run_history/                运行历史

scripts/
  quality_gate.py             聚合质量门禁
  check_*.py                  单项检查
  build_windows.ps1           Windows EXE 构建脚本

tests/
  test_*.py                   pytest 测试套件

docs/
  ALPHA_PRODUCTION_DIAGNOSIS_20260522.md
  CODE_REVIEW_TASK_EXECUTION_20260522.md
  ARCHITECTURE_MODULARIZATION_TASK_PROGRESS_20260525.md
```

## 安全原则

- 不提交真实 BRAIN 凭据。
- 不在命令行参数里传用户名和密码，避免进入 shell 历史。
- 默认用 `BRAIN_USERNAME`、`BRAIN_PASSWORD` 或 `BRAIN_TOKEN` 环境变量。
- 自动提交默认关闭。
- 提交前必须通过官方 Check、相似度、质量门槛和提交策略。
- 敏感信息扫描是质量门禁的一部分。
- 如果怀疑凭据暴露，立即重置 BRAIN 密码或 token。

## 常见问题

### Web 跑得很快，是不是已经做了真实官方回测？

不一定。候选生成、本地评分和部分检查可以很快完成；真实官方 Simulation 需要调用 BRAIN API、排队、轮询并等待结果。判断是否发生官方回测，要看运行事件、Simulation id、官方状态和提交前检查记录。

### 本地 PASS 是否等于可以提交？

不等于。提交还需要官方 Check、质量门槛、相似度、云端去重、提交策略和账号状态都满足条件。Web 中的“达标”和“可提交”是不同状态。

### strict freshness 失败怎么办？

先确认凭据环境变量存在，然后刷新官方上下文：

```powershell
python fetch_official_context.py --config config/run_config.json --json
```

再运行：

```powershell
python scripts/check_official_context.py --config config/run_config.json --strict-freshness --json
```

### README 里的测试通过数量是不是永久有效？

不是。README 中的状态是最近一次本地验证证据。提交、打包或发布前，应重新运行质量门禁和测试。

## 许可证

Internal use.
