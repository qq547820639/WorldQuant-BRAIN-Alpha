...
## [ ] Task 1: ESLint/Prettier 代码质量标准化
- **Priority**: high
- **Depends On**: None
- **Description**:
...
## [ ] Task 2: TypeScript 配置现代化（baseUrl 弃用修复）
- **Priority**: high
- **Depends On**: None
- **Description**:
...
- **Acceptance Criteria Addressed**: AC-7, AC-8
- **Test Requirements**:
  - `programmatic` TR-4.1: 主 hook 文件 ≤ 400 行
...
## [ ] Task 5: CandidateTableUtils.ts 拆分
- **Priority**: medium
- **Depends On**: Task 1
- **Description**:
...
- **Acceptance Criteria Addressed**: AC-7, AC-8
- **Test Requirements**:
  - `programmatic` TR-5.1: 拆分后每个文件 ≤ 500 行
...
- **Test Requirements**:
  - `programmatic` TR-6.1: `npm run build` 构建时间 ≤ 3 秒
...
- **Test Requirements**:
  - `programmatic` TR-7.1: `npx vitest run` 所有测试通过（0 失败）
  - `programmatic` TR-7.2: 快照测试全部通过或正确更新
  - `programmatic` TR-7.3: 集成测试全部通过
